  ...
  return (
    <section className="relative">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 flex flex-col gap-4 justify-center items-center min-h-screen">
        <div className='flex flex-col justify-between items-start pt-40 gap-28'>
          <div className="flex flex-col items-center w-full justify-center">
            <Image src="/images/medicine-mockup.png" alt={medicine.name} width={300} height={300} className='max-w-[30%] pb-2' />
            ...
            {hasPartnerPrices ? (
              <div className='flex items-center gap-6'>
                <p className='font-bold text-2xl text-gray-600'>
                  <span className='font-medium text-sm text-gray-600'>De:</span> R$ {Math.min(...medicine.PartnerPrice.map(pp => pp.price)).toFixed(2)}
                </p>
                <p className='font-bold text-2xl text-[#2dbfc7]'>
                  <span className='font-medium text-sm text-gray-600'>Até:</span> R$ {Math.max(...medicine.PartnerPrice.map(pp => pp.price)).toFixed(2)}
                </p>
              </div>
            ) : (
          ...
          <div className="flex items-center justify-between w-full gap-10">
            <div className='max-w-[50%] w-full'>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">EAN:</span> {medicine.ean}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Fabricante:</span> {medicine.manufacturer}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Princípio Ativo:</span> {medicine.activeIngredient}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Necessita Receita:</span> {medicine.needRecipe ? 'Sim' : 'Não'}</p>
            </div>
            <div className='max-w-[50%] w-full'>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Classe:</span> {medicine.class}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Categoria:</span> {medicine.category.name}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Tipo:</span> {medicine.type.name}</p>
              <p className="border-b border-b-gray-300 pb-3 pt-1 text-gray-600"><span className="font-bold">Especialidades:</span> {medicine.specialties.map(s => s.name).join(', ')}</p>
            </div>
          </div>
        </div>
        {hasPartnerPrices && (
          <div className="space-y-4 w-full mt-10">
            <h3 className='font-bold text-3xl mb-6'>Preços por Parceiro</h3>
            <div className="flex flex-col gap-4 ">
              {medicine.PartnerPrice.map((partnerPrice) => (
                <div key={partnerPrice.id} className="flex border justify-between border-gray-200 rounded-lg px-8 py-8 shadow-sm hover:bg-zinc-100 transition-all">
                  <div className="flex items-center gap-12">
                    ...
                    <div className='flex flex-col'>
                      <span className="font-semibold text-xl">{partnerPrice.partners ? partnerPrice.partners.name : 'Parceiro'}</span>
                  ...
                  <p className="text-2xl font-bold text-[#2dbfc7]">R$ {partnerPrice.price.toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
